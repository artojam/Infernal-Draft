...
public class Pathfinding : MonoBehaviour
{
    ...
    public static byte[] GenerateGridFromTilemap(
        Tilemap tilemap,
        Vector2Int posRoom,
    ...
    {
        byte[] gridBuffer = new byte[_sizeRoom.x * _sizeRoom.y];
        BoundsInt bounds = tilemap.cellBounds;
        Vector2Int offsetRoom = new Vector2Int( 2, 3); //(_sizeRoom / 2);

        for (int x = 0; x < _sizeRoom.x; x++)
        {
            for (int y = 0; y < _sizeRoom.y; y++)
            {
                Vector3Int tilePosition = new Vector3Int(x - (_sizeRoom.x / 2), y - (_sizeRoom.y / 2) + 1, 0);
                TileBase tile = tilemap.GetTile(tilePosition);
                gridBuffer[x * _sizeRoom.y + y] = tile == Wall ? (byte)255 : (byte)0;
            }
        }
        return gridBuffer;
    }
